...
### 0단계: 프로젝트 템플릿 복사하기 (준비 작업)

새로운 기능 개발을 시작하기 전에, Lite-SPA의 기본 뼈대가 되는 `template/` 폴더를 복사하여 자신만의 새 작업 디렉토리를 만듭니다.

1. 이 레포지토리의 `template/` 폴더 안의 모든 파일과 폴더를 통째로 새 폴더(예: `memo`)로 복사합니다.
2. 새로 구성한 폴더의 내부 구조는 다음과 같아야 합니다:
   ```
   📁 memo/
   ├── index.html       (앱 셸)
   ├── app.js           (라우팅 및 비즈니스 로직)
   ├── store.js         (전역 상태 선언)
   ├── i18n.js          (다국어 번역 사전)
   └── 📁 pages/        (개별 페이지 조각 폴더)
       ├── home.html
